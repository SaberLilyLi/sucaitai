...
| 素材库 | 本地 `data/library.json`，支持项目、标签、备注，无需数据库 |
| 商品对比 | 勾选 2–4 个商品，横向对比价格、销量、发货、素材和归档信息 |
| 批量导出 | 将已拉取的本地素材按商品打成 ZIP，同时附带素材清单 |
...
### `POST /api/products/export-media`

把已拉取的本地素材按商品目录打成 ZIP。请求 body：

```json
{ "ids": ["1234567890", "2345678901"] }
```

若选中商品没有本地素材，接口会返回提示，需先拉取详情素材。
